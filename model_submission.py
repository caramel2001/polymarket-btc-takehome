...
class ModelSubmission(Model):
    ...
    def on_tick(self, tick: Tick) -> Signal | None:
        ...
        if self._pos_side is not None:
            ...
            # Reversal exit (only when not near resolution — don't churn late)
            if ttr > RESOLUTION_HOLD_TTR and self._reversed(tick):
                return self._close()
        ...
        self._pos_side = side
        self._pos_size = size
        self._entry_price = tick.up_ask if side == Side.UP else tick.up_bid
    ...
    def _directional_score(self, tick: Tick) -> tuple[float, Side | None]:
        ...
        # All horizons must agree in direction
        signs = {1 if r > 0 else -1 for r in (r5, r15, r30)}
        if len(signs) != 1:
            return 0.0, None
